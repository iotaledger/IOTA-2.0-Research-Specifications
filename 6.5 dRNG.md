...
For brevity denote `TIMESTAMP_CUTOFF` by $\Delta_C$ and `ApplicationWindow` by $\Delta_A$.Assume that a committee should be formed at the time $t_F$ (which is known to all interested nodes; it is defined on the protocol level). Assume further that the time $t_F$ is in the epoch $E$ i.e., $t_F$ $\in$ $[t_{E-1}$,$t_E]$. Then the Mana of all of the nodes is calculated with respect to the active consensus Mana vector from a time $t_{E-2}$ which is a consensus Mana balance from two epochs before $t_F$. The committee selection process starts with the opening of the application message window at the time $t_S$, where $t_S$ = $t_F$-$\Delta_A$ -2$\Delta_C$. For as long as the application window is open, nodes can issue application messages. See the subsection "6.5.3.1.1 Application message sending - default algorithm" for the proposed algorithm of putting application messages (which is not enforceable). 
