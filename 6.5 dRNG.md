...
The distributed random number generator specification depends on the following specification:
...
| -----| ------ | ----------- |----------- |
| `rnPeriod`  | float | Random number is produced every`rnPeriod` seconds|  20 [sec]|
...
| `committeeUpdate` |float | Period of committee update|  1 [day] = 86400 [sec]|
|   `applicationWindow`   | float | Time window of the ''application'' message submission| 120 [sec] |
|  `TIMESTAMP_CUTOFF`     | float |  Message timestamp cutoff (Assuming the node is in sync, the time after which point the node will receive no new messages with a particular timestamp which will be finalized)|  2`DLARGE` +`W`= 90[sec]|

Table 6.5.1. dRNG parameters. Description and suggested values.


Note: For more information on the `TIMESTAMP_CUTOFF`  see section [Section 4.2 -  Timestamps](./4.2%20Timestamps.md)
