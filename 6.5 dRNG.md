...
Assume that a committee should be formed at the time $t_{comm}$ (which is known to all interested nodes; it is defined on the protocol level). Assume further that the time $t_{comm}$ is in the epoch $E$ i.e., $t_{comm} \in [t_{E-1},t_E)$. Then the Mana of all of the nodes is calculated with respect to the active Mana vector from a time $t_{E-2}$ which is a Mana ballance from two epoches before $t_{comm}$.
...
After a successful committee selection -confirmed at the time $t_{comm}$ with respect to node's local clock, the DKG phase starts. In this phase, the committee nodes exchange the *Deal messages* to produce a public/private collective key. Nodes should proceed with DKG message exchange as soon as the committee selection is confirmed (time $t_{comm}$).
