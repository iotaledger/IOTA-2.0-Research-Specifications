...
Assume that a committee should be formed at the time $t_F$ (which is known to all interested nodes; it is defined on the protocol level). Assume further that the time $t_F$ is in the epoch $E$ i.e., $t_F$ $\in [t_{E-1},t_E)$. Then the Mana of all of the nodes is calculated with respect to the active Mana vector from a time $t_{E-2}$ which is a Mana ballance from two epoches before $t_F$. The application process starts at the time $t_S$
