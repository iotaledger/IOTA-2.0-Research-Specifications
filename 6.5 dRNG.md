...
| -----| ------ | ----------- |
| `RNPeriod`  | integer | Random number is produced every`RNperiod` seconds|  20 [sec]|
...
| `CommitteeUpdate` |integer | Period of committee update|  1 [day] = 86400 [sec]|
|   `ApplicationWindow`   | integer | Time window of the ''application'' message submission| 120 [sec] |
|  `TIMESTAMP_CUTOFF`     | integer |  Message timestamp cutoff (Assuming the node is in sync, the time after which point the node will receive no new messages with a particular timestamp which will be finalized)|  2`DLARGE` +`W`= 90[sec]|
