...
The dRNG module depends on the **Mana module** since it uses the Consensus Mana (cMana) vector as a measure of trustworthiness. Specifically, it uses the list of the top Mana holders to select a committee to produce the random numbers. During the committee selection, we do not assume a perfect agreement on the cMana values, i.e., different nodes can have slightly different perceptions of the cMana values of other nodes (due to the different local clocks). Obtaining consensus on the cMana values is the central part of this documentation. 
...
The Tangle graph after Coordicide lacks a natural reference point to calculate cMana values. Notice that this reference point is quite straightforward to define in blockchain-based ledgers (every multiple of $k$ block height, for instance). We introduce then a reference point using timestamps of the transactions, which are objective and consequently not subject to local clocks discrepancies.
