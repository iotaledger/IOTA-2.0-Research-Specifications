...
- **simplified payment verification (SPV):** Finality gadget statements could be used to enable (ad-hoc) nodes to verify the finality of messages without the need to download the entire Tangle. This feature can be implemented into future 

## 4.2.2 Preliminaries
### 4.2.2.1 Requirements
1. Nodes must have consensus on which messages are valid.
2. Nodes must have consensus on the timestamp asscoated to each message.
3. Eventually, for every time `t`, eventually, any new message issued with timestamp `t` will be too far in the past and will be orphaned.
