...
| -----| ------ | ----------- |
|`W` | integer | window ~1 minute. Require w>2D|
| `DELTA` | integer | max age difference in consecutive timestamps. Require DELTA>w+D|
| `TW` | integer | max difference between message timestamp and transaction timestamp|
| `EPOCHLENGTH` | integer | the length of each epoch ~ 1 hour|
...
In order for a message to be eligible for tip selection, the timestamp of every message in its past cone (both weak and strong) must satisfy certain requirements. These requirements fall into two categories: objective and subjective. Objective criteria only depends only information written direclty in the tangle immediately upon solidification.  Thus all nodes immediately have consensus on the objective criteria.  In this section, we will discuss these objective criteria.

The quality of the itmestamp is a subjective criterion because it is based on the soldification time of the message.  Thus, nodes must use a consens algorithm, e.g. FPC, to decide which messages should be rejected based on subjective criteria. Specifically, nodes will use FPC to vote on whether or not a timestamp is before `W` plus the arrival time. Therefore, we can guarantee that all timestamps are approximately correct.  
