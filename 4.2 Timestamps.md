...
Having consensus on the creation time of messages enables not only total ordering but also new applications that require certain guarantees regarding time. Specifically, we may use message timestamps to enforce timestamps in transactions, which may also be used in computing the mana associated to a particular node ID. 
...
2. Nodes must have consensus on the timestamp associated to each message.
...
+ `CurrentTime => t + W + 2*DLARGE`, i.e. wait ~1.5 minutes
+ `SyncStatus = TRUE`
Indeed, after `W + 2*DLARGE` all messages which arrive will be considered bad with level of knowledge 3: see [???](). If the node is in sync, then it will have received all old messages which will be confirmed. 
...
### 4.2.4.1 Not in Sync
Any node not in sync will receive messages much later than than the rest of the network.  Thus, all messages will appear to have inaccurate time stamps and will be wrongfully rejected by the algorithms in [???](). Thus nodes will not actively participate in any voting until its status is in sync, see Section 4.2.5. 
...
What about the converse situation? Being out sync will only delay the arrival of a message.  If a node receives a message with timestamp satisfying `timestamp+W>arrivalTime`, this condition would also be satisfied for all nodes which received the message earlier.  Thus, if a node is out of sync andd is receiving messages later than everyone else, if this node likes a timestamp, all other notes will hve already liked it. Therefore, nodes will not like timestamps which were previously rejected by most of the network.  
...
+ In order to signal to clients that its perception is healthy
+ When to issue messages (nodes out of sync should not issue messages, lest they are added to the wrong part of the tangle)
+ When a node is out of sync, it must schedule messages at a faster rate in order to catch up with the network
+ To optimize FPC: nodes should not query while syncing, instead relying on the approval weight. 

### 4.2.5.2 Tangle Time

Every DLT is a clock, or more specifically a network of synchronized clocks.   This clock has a natural correspondence with "real time".  If the DLT clock differs from "real time", then the we can conclude that our DLT clock is off from all the other clocks.  
...
FUNCTION Time = TangleTime
 ...
 ```
Thus tangle time is the last time stamp to have been confirmed and added to the ledger. Tangle time cannot be attacked without controlling enough mana to accept incorrect timestamps, making it a reliable, attack resistant quantity.
