...
Because of the FPC voting, and the levels of knowledge, with high probability, we can be sure that for any time `t`, no node can issue a message with timestamp `t` after `t + W + 2 DLARGE`, because after this time, the message would be considered to be bad with level three knowledge: see [???]().  Thus, after approximately ~1.5 minutes, the number of messages of a particular timestamp cannot be altered.  
