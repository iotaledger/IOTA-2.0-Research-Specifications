---
description: The Realities Ledger State deals with double spends and leverages on certain properties of UTXO.  
...
- UTXO: see the Section on [UTXO DAG](#5.2.4 The UTXO DAG) as well as [5.1 UTXO](./5.1UTXO).
...
The UTXO model and the concept of solidification, see section [4.4 Solidification](./4.4Solidification.md), makes all non-conflicting transactions converge to the same ledger state no matter in which order the transactions are received. Messages containing these transactions could always reference each other in the Tangle without limitations.
...
This assignments captures the essence of weak and strong parents, see [4.3 Tip Selection Specification](./4.3TipSelectionAlgorithm.md).  Strong arrows pick up the dependencies of the whole past cone, where as the weak arrows only penetrate to the paylaod of the parent, ignoring the history of the parent.  
