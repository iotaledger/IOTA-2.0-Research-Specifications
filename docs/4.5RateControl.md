---
description: In order to enable the machine-to-machine economy, IOTA does not allow either mining race nor fees, which makes an explicit rate control mechanism necessary to act as an emergency br during spam attacks, by slowing down the rate of messages a node can issue.
