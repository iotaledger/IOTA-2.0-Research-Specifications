...
The mechanisms described act as an emergency break/docs/2.4DataFlow.mdculty should be small enough not to hamper performance.  Finer controls on the access are developed in [Section 4.6 - Congestion Control](./4.6CongestionControl), which regulate network traffic during normal periods of congestion.
