...
Once the message has successfully passed the message parser checks and is solid, it is enqueued into the outbox for scheduling (see [Section 2.4 - Data Flow](./2.4 Data flow.md)). The outbox is logically split into several queues, each one corresponding to a different node issuing messages. In this section, we describe the operations of message enqueuing (and dequeuing) into (from) the outbox.
...
+ Node's `syncStatus` flag is set to `FALSE` (see [Section 4.2 - Timestamp](./4.2 Timestamps.md)).
