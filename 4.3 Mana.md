...
Let us denote $\text{BM}(t)$ as the Base cMana of a given node. Then the **Consensus Mana**  $M(t)$ of this node at time $t$ is the EMA of $\text{BM}(t)$ and defined as following. It is initialized with $M(t_0) = 0$ at the time of creation $t_0$ of the node.  The time of creation of a node is the timestamp in the salt declaration message of the node. For any $t>t_0$ we define recursively

$$
TO COME
$$

This recursive uppate must be perfomed at all points in time $t$ where the value of $BM(t)$ changes. The update shall also be done periodically to maintain an accurate perception of the cMana state.
...
When outputs are consumed by a transaction, a certain amount of Pending Mana is pledged to the node `AccessManaNodeID`. The Pending Mana $\text{PM}_{t_o, S}(t)$ at time $t$ of a given unspent output of value $S$ that was created at time $t_0$ (timestamp of transaction)  is defined as

$$
\text{PM}_{t_o, S}(t) = S \cdot (1- \exp(-\gamma (t-t_0)),
$$
...
$$
\text{BM}(t) = \text{BM}(t_0) \exp(-\gamma(t -t_0)),
$$
for $t\in [t_0,t_1]$.
