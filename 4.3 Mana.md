...
Additionally, we define the Active Consensus Mana of a Node A in Epoch E as
...
Therefore, even if node `node` has Consensus Mana greater than zero at at certain epoch, it can be considered dormant in case it did not issue any message during the same epoch. All nodes that have Active Consensus Mana in a certain epoch will form the Active Consensus Mana Set `ACMS(epoch)`.
