...
Both calculations defined above (cMana and Active cMana on an epoch $E$) can only be carried out when epoch $E$ is finalized&mdash;that is, at least `TIMESTAMP_CUTOFF` (see timestamp spec) units of time after $t_E$&mdash;to make sure that no more valid messages belonging to the epoch will appear in the network.
