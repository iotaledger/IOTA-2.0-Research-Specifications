...
Therefore, even if node `node` has consensus Mana greater than zero at a certain epoch `epoch`, it can be considered dormant in case it did not issue any message during `epoch`. All nodes that have active consensus Mana in `epoch` will form what we call the active consensus Mana set of `epoch`, or `ACMS(epoch)`.
