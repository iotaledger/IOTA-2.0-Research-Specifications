...
* **Rate control**: The throughput (in bytes per second) of each node is dependent on the access Mana held.
* **Neighbor Selection**:  Nodes only establish a connection with nodes of similar consensus Mana.
* **Fast Probabilistic Consensus**: The selection probability for a node during a voting query is proportional to the consensus Mana held.
* **dRNG**: The dRNG committee members must be top consensus Mana nodes.
...
On the other hand, cMana is updated only relatively to the end of each epoch <!--(see ?????LINK)-->. The reason behind this is that epochs are objective, i.e. all nodes agree on which epoch a certain message (or transaction) belongs to. Thus, since nodes with high probability  have the same transactions, given that they are older than `DELTA` <!--(see ?????LINK)-->, they will consequently agree—with high probability–about the cMana vector relative to these transactions and epochs.
...
The consensus Mana of a node is only calculated and stored relatively to the end of each epoch <!--(see LINK?????????)-->. Only the values of the last `MAX_STORED_EPOCHS` epochs are stored. Thus, if $t_E$ is the time of the end of epoch $E$, to update the cMana vector from epoch $E-1$ to $E$, a node must perform the following algorithm:
...
Both calculations defined above (cMana and Active cMana on an epoch $E$) can only be carried out when epoch $E$ is finalized—that is, at least `TIMESTAMP_CUTOFF` (see timestamp spec) units of time after $t_E$—to make sure that no more valid messages belonging to the epoch will appear in the network.
...
The Mana is an extension of the ledger state, hence its calculation depends on the ledger state perception of the node. Snapshotting is the mechanism that stores older ledger states and prunes unnecessary messages. Together with the ledger state, aMana and cMana vectors (and the reference time relative to them) are also saved, since a certain ledger state reflects a certain aMana and cMana distribution in the network. 

When a node joins the network, it will query other nodes to get their snapshot file, containing a `aMana Snapshot Vector`, a `cMana Snapshot Vector`, and two `ACMS` (one for each of the last two snapshotted epochs) that will be used as initialization data. In case of a large database, initial synching and solidification may be a computationally heavy task due to the sheer amount of messages in the tangle. Access and consensus Mana calculations only adds to this burden. Then, some additional mechanism may be needed to be implemented, such as for example delaying aMana and cMana calculation until the initial syncing is finished.
