...
Since this Mana will be updated frequently, we do not recomend to update the Base Mana Vectors based on the Snapshot File. Instead, a more recent Base Mana Vector should be locally stored by the components using the Access Mana and used as a outdated Base Mana Vector database.

### Example 2 - Access Mana

Suppose a node has a Base Mana Snapshot State stored, relative to time 0, and needs to update its Access Mana Vector every second $i$, to include the Mana relative to the set of transactions $S_i$ booked in the period $$. To do that, the node will call the function

Get_Mana(t,last_update_time,set_of_transactions)
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTgzODIyNDIyOCwyMTEwMjUzMDc1LDE5MD
M4NDgwOTUsMjE0NzIxNTcxLC0xODM4NDczMjY0LC05MjQwNTg0
NTJdfQ==
-->