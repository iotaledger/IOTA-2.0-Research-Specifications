...
* The `Snapshot_State` is the database store in each node's snapshot file;
* `Set_of_Transactions`must be compatible with the `Snapshot_State` (i.e., the transactions in`Set_of_Transactions` can only spend UTXOs from  `Snapshot_State` or `Set_of_Transactions` itself);
...
Update_Base_Snapshot_Mana(t,T,Set_of_Transactions,Snapshot_Mana)
```
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTYxMzY2NjkzOSwtMTMwNDk3NTU0OCwzMz
