...
Any permissionless system needs a Sybil protection mechanism. In coordicide, this is done by forcing every node to create a node identity (see also **Subsection Peer Discovery** LINK?????). Two *Difficult-to-obtain* resources are linked to each node identity; we call them **aMana** (**Access Mana**) and **cMana** (**Consensus Mana**). 

Both Manas are, perhaps, best visualized as tools that have multiple roles in the network. They are related to&mdash;but also remain separate from&mdash;the IOTA token. When a value transaction is processed, a certain amount of aMana and cMana&mdash;dependent on the amount of IOTAs moved by the transaction&mdash;will be "pledged" to specified node IDs. The Mana pledged to each node ID must be stored as an extension of the ledger state. The only way a node can obtain Mana (either aMana or cMana) is to convince some token holders to pledge Mana to it. In this sense, Mana can be thought as Delegated Proof of Token Ownership. It provides adequate Sybil protection because it is difficult to acquire in arbitrary amounts.  
