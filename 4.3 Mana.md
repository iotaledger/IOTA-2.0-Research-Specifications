...
>[SEB] Missing: 
> 1. active Consensus Mana 
> 2. Consensus Mana calculated at epochs

This section introduces  **Access Mana** and **Consensus Mana** and determines for which modules each of the two types of Mana must be used.

Any permissionless system needs a Sybil protection. In coordicide this is done via node identities, where every node must create a node identity, see also **Subsection Peer Discovery**.  *Difficult-to-obtain* resources are linked to each node identity  that we call **aMana** (**Access Mana**) and **cMana**  (**Consensus Mana**). 

Both Manas are perhaps best thought of as tools that are used in various roles in the network. They are related to but also remain separate from the IOTA token. When a value transaction is processed, a certain amount of aMana and cMana will be "pledged" to  specified node IDs. This quantity is related to the amount of IOTA moved by the transaction. The Mana pledged to each node ID must be stored as an extension of the ledger state. The only way a node can obtain Mana (aMana or cMana) is to "convince" some token holders to pledge Mana to their node. In this sense, Mana is Delegated Proof of Token Ownership. It provides adequate Sybil protection because it is difficult to acquire in arbitrary amounts.  

Mana is used as Sybil protection in different modules, namely congestion control, neighbor selection, consensus protocol, dRNG, and approval weights. Due to the different nature and requirements of each module, it is natural to use different types of Mana.

Consensus Mana should be seen as the "Mana" that is responsible for the security of the system. In particular, it must be used for the consensus protocol, the dRNG module, and the approval weight. Moreover, it serves as an eclipse protection mechanism in the autopeering in guaranteeing that nodes of similar Mana do peer with each other. Access Mana is used to grant access to network through the congestion control; the amount of data a node can add to the Tangle   is proportional to its Access Mana.

We give a short overview on which kind of Mana is used in which module:

* **Congestion control**: The throughput of each node is limited by its Access Mana.
* **Neighbor Selection**: Only nodes with similar Consensus Mana can peer.
* **Voting**: The voting power of a node is proportional to the its Consensus Mana.
* **dRNG**: The dRNG committee consists of the top Consensus Mana nodes.
 

# 4.3.2. Detailed Design

 Each transaction must have an `AccessManaNodeID` and `ConsensusManaNodeID` field to determine which node to pledge these two types of Mana to. Both of these fields consist of a node ID. Note that Access Mana and Consensus Mana do not have to be pledged to the same node. 

 We use the following constants:
 

 **Table 4.3.1-1 Mana Parameter Default Values**

| Parameter  |      Value     | Description |
|----------|:-------------:| -------------:|
| `DECAY` | 0.00192541  | Generation and Decay coefficient for Base aMana |
|  `EMA_COEFF_A` |  ? | Moving average  coefficient  Access Mana  |
| `EMA_COEFF_C` |  0.00192541  | Moving average  coefficient  Consensus Mana  |


# 4.3.2.1 Exponential Moving Average
An exponential moving average (EMA) is a type of moving average that places a greater weight and significance on the most recent data points. An EMA is characterized by a coefficient $c >0$ that represents the degree of weighting decrease or smoothing factor. The higher this factor $c$ the faster older values are discounted. 
The EMA of a function $X(t)$ is defined as follow:

$$
   \text{EMA}_X(t) 
    = c \int_{-\infty}^t X(s) e^{-c(t-s)}ds.
$$
Let us warn here that most literature and references on EMA treat discrete versions. As this may lead to confusion and possible different implementation, any implemenation must follow the definition above. 

The weighting for older datum decreases exponentially in time, however, never reaching zero. For this reason EMAs shall be  calculated recursively using the following formula. For any $t, \triangle t >0$ we have

$$
\text{EMA}_X(t+\triangle t) = e^{-c \triangle t} \text{EMA}_X(t) + c \int_0^{\triangle t} X(t+s) e^{-c(\triangle t -s)} ds.
$$
...
# 4.3.2.2 Consensus Mana
The Consensus Mana (cMana) of a node is an exponential moving average of the  **Base cMana**. The base cMana of a node `nodeID` at time `time` is defined as the sum over all unspent outputs at time `time` of transactions  with  `ConsensusManaNodeID = nodeID`. 

An exponential moving average (EMA) is a type of moving average that places a greater weight and significance on the most recent data points.  More precisely, the weighting for older datum decreases exponentially in time, however, never reaching zero. For this reason EMAs are mostly calculated recursively. An EMA is characterized by a coefficient $\alpha \in [0,1]$ that represents the degree of weighting decrease or smoothing factor. The higher this factor $\alpha$ the faster older values are discounted. The value of $\alpha$ is set to ?.

Let us denote $\text{BM}_t$ as the Base cMana of a given node. Then the **Consensus Mana**  $M_t$ of this node at time $t$ is the EMA of $\text{BM}_t$ and defined as following. It is initialized with $M_{t_0} = 0$ at the time of creation $t_0$ of the node.  The time of creation of a node is the timestamp in the salt declaration message of the node. For any $t>t_0$ we define recursively

$$
TO COME
$$

This recursive uppate must be perfomed at all points in time $t$ where the value of $BM_t$ changes. The update shall also be done periodically to maintain an accurate perception of the cMana state.


# 4.3.2.3 Access Mana
Similar to cMana, aMana is the EMA of Base aMana. The definition of Base aMana is more involved and uses the concepet of Pending Mana.
When outputs are consumed by a transaction, a certain amount of Pending Mana is pledged to the node `AccessManaNodeID`. The Pending Mana $\text{PM}_t(t_o, S)$ at time $t$ of a given unspent output of value $S$ that was created at time $t_0$ (timestamp of transaction)  is defined as

$$
\text{PM}_t(t_o, S) = S \cdot (1- \exp(-\gamma (t-t_0)),
$$
where $\gamma$ is the generation coefficient `DECAY`.

Whenever a transaction pledges Access Mana to a node, the sum of all Pending Manas of the spent outputs is added to the Base aMana of the node.  
This pledge is never revoked, as opposed to Base cMana. However, the Base aMana of a node will *decay* over time as follows. If no new Pending Mana was pledged to a node in the interval $[t_0,t_1]$  the value of the Base aMana $\text{\text{BM}}_t$ is described as

$$
\text{BM}_t = \text{BM}_a \exp(-\gamma(t -t_0)),
$$
for $t\in [t_0,t_1]$.

The **Access Mana** is the EMA of the Base aMANA with coefficient `EMA_COEFF_A`.
