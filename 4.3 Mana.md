...
* Neighbor Selection: Nodes with similar Total Mana (i.e., Access + Consensus) will be neighbors with high probability.
...
We begin defining the *Base Mana Vector* relative to a certain Tangle State as a vector that assigns a *Base Mana 1*, *Base Mana 2*, *Effective Base Mana 1* and *Effective Base Mana 2* values to each known node. Here we call a Tangle State any triple `(Snapshot_State, Set_of_Transactions, time)`, where:

* The `Snapshot_State` is the database stored in each node's snapshot file;
* `Set_of_Transactions`must be compatible with the `Snapshot_State` (i.e., the transactions in`Set_of_Transactions` can only spend UTXOs from  `Snapshot_State` or `Set_of_Transactions` itself) and compatible to itself (i.e., no double spendings);
*  `time` must be larger or equal than the timestamps of all the transaction in`Set_of_Transactions` .


Notice that this might not be the canonical definition of a Tangle State. Below we find an example of a Base Mana Vector:
...
The Base Mana Snapshot State will only be updated when transactions are snapshotted. Nevertheless, an updated vector of the Base Mana State can be provided whenever the function is called as a consequence of external events (like the DRNG or the Rate Control modules) not related to the snapshotting process.


#### Example 1 (Snapshoting transactions): 
Suppose a node has a snapshot file that was relative to a certain point in time `T` in the past. If the node wants to update it to a certain time `t` in order to add the set of transaction `Set_of_Transactions` (note that here, `t` must be the largest timestamp among `Set_of_Transactions`, and `T` must be the largest timestamp among the snapshotted transactions), it will call the functions:

```
Update_Snapshot_Ledger_State(Snapshot_Ledger_State,Set_of_Transactions)
Update_Base_Snapshot_Mana(Base_Mana_Snapshot_State,Set_of_Transactions,t,T)
```
In this case, in general, all the databases in the snapshot with will be modified (i.e. , the Ledger State, both Base Mana vectors, both Effective Base Mana vectors and the time it is relative to).
...
Obs: `Set_of_Transactions` can be an empty list, in the extreme case of not having transactions issued for a long time. In this case the variable `t` should be set to a point in time close to the node's local clock. 
...
Obs 4: If a node wants to build the Mana database from scratch, i.e, without a already initialized Base Mana State, it suffices to set all Base Mana and Effective Base Mana States to zero and call the function `Get_Base_Mana(t,last_update_time,Set_of_Transactions)` setting `last_update_time` to any value of time.
...
```
```
function Get_Base_Mana(t,last_update_time,Set_of_Transactions,Outdated_Base_Mana_State):
    (Base_Mana_1,Base_Mana_2,Eff_Base_Mana_1,Eff_Base_Mana_2) = Outdated_Base_Mana_State
...
<!--stackedit_data:
eyJkaXNjdXNzaW9ucyI6eyJMNnRnNG44c2FNZTFLeHVxIjp7In
N0YXJ0Ijo2NSwiZW5kIjo4ODksInRleHQiOiJUaGlzIHNlY3Rp
b24gZGVmaW5lcyB0d28gdHlwZXMgb2YgKkJhc2UgTWFuYSogY2
