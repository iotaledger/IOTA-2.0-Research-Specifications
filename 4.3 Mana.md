...
Suppose that the last reference time is $t$ and the transaction timestamp is $t-s$. The update must be done in two steps:
...
Suppose that the last reference time is $t-s$ and the transaction timestamp is $t$. The update must be done in three steps:
...
Suppose that the last reference time is $t-s$ and new one is $t$. The update must be done in two steps:
