...
Suppose that the last reference time is $t-s$ and new one is $t$. The update must be done as specified below:

*  **Updating the cMana with respect to time**. Suppose that the last cMana vector is $\text{Old\_cMana}$. We update all cMana entries as follows:
