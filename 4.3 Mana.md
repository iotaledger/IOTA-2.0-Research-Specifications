...
Both calculations defined above (cMana and Active cMana on an epoch $x$) can only be carried out when epoch $x$ is finalized&mdash;that is, at least `Delta` units of time after $t_x$&mdash;to make sure that no more valid messages belonging to the epoch will appear in the network.
