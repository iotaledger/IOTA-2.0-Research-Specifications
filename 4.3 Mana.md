...
### 4.3.4.1.1  Pledging the aMana relative to a transaction with timestamp smaller than the last reference time
 ...
 The update must be done in four steps (notice that in certain situations described below, some steps must be skipped):

* **Base aMana pledging**. Suppose that the new transaction consists of $m$ inputs $I_j$ (of value $x_j$), and pledges aMana to a node $N$. We update the base aMana of node $N$ by adding to its base aMana the value
...
* **Pledging aMana**. Suppose that the aMana is (after last step) relative to time $t$ and the transaction being added has a timestamp $t-\delta$. Then, we update the aMana vector adding to each $\text{aMana(Node\_i)}$ entry the term:
...
### 4.3.4.1.2  Pledging the aMana relative to a transaction with timestamp larger than the last reference time


 The update must be done in four steps (notice that in certain situations described below, some steps must be skipped):

* **Base aMana update with respect to time**. Suppose that the outdated base aMana $\text{Old\_Base\_aMana}$ is relative to time $t-s$ and we want to update it to time $t$. In this case, we update the aMana entries as follows:
...
* **Updating the aMana with respect to time**. Suppose that the outdated aMana $\text{Old\_aMana}$ is relative to time $t-s$ and we want to update it to time $t$. In this case, we update the aMana entries as follows:
...
### 4.3.4.1.3  Updating the aMana to a time $t_1>t_0$ without any new aMana pledging.
