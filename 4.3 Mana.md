...
* Voting: the weight of a vote should be proportional to the Consensus Mana owned.
* DRNG: the DRNG committees will be constituted by the high Consensus Mana nodes.


In the future, we expect Mana to be only a specific aspect of a more generic reputation system which might include other criteria, such as penalties for misbehavior or incentives for helping the network. Also, criteria which are external to the system (informally, any sort of “real-world importance”) may be used.
...
We begin defining the *Base Mana Vector* relative to a certain Tangle State as a vector that assigns a *Base Mana 1*, *Base Mana 2*, *Effective Base Mana 1* and *Effective Base Mana 2* values to each known node. Here we call a Tangle State any triple `(Snapshot_File, set_of_transactions, time)`, where `set_of_transactions`must be compatible with the `Snapshot_File` and `time` must be larger or equal than the timestamps of all the transaction in`set_of_transactions` (notice that this might not be the canonical definition of a Tangle State).
...
* $n_{\text{max}}$ (`n_max`)- largest acceptable period of time without updating the Effective Base Mana vectors - (for now, we use $n_{\text{max}}=30$ minutes) 
...
The algorithm is not finalized yed. Now, suppose that each transaction $j$ in `set_of_transactions`, has a timestamp $t-\delta_j$. If the Base Mana 1 balance of a node $i$ instantly before we added this transaction was $\text{Mana_1}^{-}(\text{Node_i})$ and the new Base Mana 1 balance, after the addition of this transaction is $\text{Mana_1}^{+}(\text{Node_i})$, then we update the Effective Base Mana 1 vector adding to each $\text{Effective_Mana_1(Node_i)}$ entry the term:

$$
(1-(1-\alpha)^{\delta_j})[\text{Mana_1}^{+}(\text{Node_i})-\text{Mana_1}^{-}(\text{Node_i})]
$$
...
where $\alpha$ is the same parameter set for the Effective Base Mana 1 and $\gamma$ is the same parameter used to update the Base Mana 2 vector. Notice that here, the value of $\text{Mana_2(Node_i)}$ used is the one already updated. 
...
Obs 4: If a node wants to build the Mana database from scratch, i.e, without a already initialized Mana State, it suffices to set all Mana and Effective Mana States to zero and call the function `Get_Mana(t,last_update_time,set_of_transactions)` setting `last_update_time` to any value of time.
Obs 5: When calling `Get_Mana(t,last_update_time,set_of_transactions)` 

```
function Update_Mana_2_time(t,last_update_time):
    n = t-last_update_time
    for each node i:
        Mana_2(i) = Mana_2(i)*exp(-decay*n)  
```
```
function Update_Eff_Mana_1_time(t,last_update_time):
    n = t-last_update_time
    for each node i:
        Eff_Mana_1(i) = (1-EMA_coeff)**n*Eff_Mana_1(i)
                        +(1-(1-EMA_coeff)**n)*Mana_1(i)
```
```
function Update_Eff_Mana_2_time(t,last_update_time):
    n = t-last_update_time
    for each node i:
        Eff_Mana_2(i) = (1-EMA_coeff)**n*Eff_Mana_2(i)
                        +(1-(1-EMA_coeff)**n
...
```
```
function Get_Mana(t,last_update_time,set_of_transactions):
...
<!--stackedit_data:
eyJkaXNjdXNzaW9ucyI6eyJRbFAxWk9XSFJEbURJOFpHIjp7In
RleHQiOiJJbiB0aGUgZnV0dXJlLCB3ZSBleHBlY3QgTWFuYSB0
byBiZSBvbmx5IGEgc3BlY2lmaWMgYXNwZWN0IG9mIGEgbW9yZS
BnZW5lcmljIHJl4oCmIiwic3RhcnQiOjgxMywiZW5kIjoxMDE2
