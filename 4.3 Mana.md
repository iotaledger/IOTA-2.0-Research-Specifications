...
|----------|:-------------:| -------------:|
| `DECAY` | 0.00192541  | Decay factor for Base aMana |
...
Let us denote $BM_t$ as the Base cMana of a given node. Then the **Consensus Mana**  $M_t$ of this node at time $t$ is the EMA of $BM_t$ and defined as following. It is initialized with $M_{t_0} = 0$ at the time of creation $t_0$ of the node.  The time of creation of a node is the timestamp in the salt declaration message of the node. For any $t>t_0$ we define recursively

$$
M_t = \alpha \cdot BM_t+ (1 - \alpha)^{(t-t_0)} \cdot M_{t-t_0}
$$
