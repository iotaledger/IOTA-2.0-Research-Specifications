...
Both Manas are, perhaps, best visualized as tools with multiple roles in the network. They are related to the IOTA token, but are not tokens by themselves (????? add a good explanation here) and do not interfere with the token balance in any way. When a value transaction is processed, a certain amount of aMana and cMana—dependent on the amount of IOTAs moved by the transaction—will be "pledged" to specified node IDs. The Mana pledged to each node ID must be stored as an extension of the ledger state. The only way a node can obtain Mana (either aMana or cMana) is to convince some token holders to pledge Mana to it.  Mana provides adequate Sybil protection because is difficult to be acquired in arbitrary amounts.  
