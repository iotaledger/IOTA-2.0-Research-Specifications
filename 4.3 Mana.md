...
An exponential moving average (EMA) is a type of moving average that places a greater weight and significance on the most recent data points.  More precisely, the weighting for older datum decreases exponentially in time, however, never reaching zero. For this reason EMA are mostly calculated recursively. An EMA is characterized by a coefficient $\alpha \in [0,1]$ that represents the degree of weighting decrease or smoothing factor. The higher this factor $\alpha$ the faster older values are discounted. 

Let us denote $BM_t$ as the Base cMana of a given node. Then the EMA $M_t$ of $BM_t$ is defined as following. It is initialized with $M_{t_0} = 0$ at the time of creation $t_0$ of the node.  The time of creation of a node is here the timestamp in the salt declaration message of the node. For any $t>t_0$ we define recursively

$$
M_t = \alpha \cdot BM_t+ (1 - \alpha) \cdot M_{t-t_0}
$$

This recursive uppate must be perfomed at all points in time $t$ where the value of $BM_t$ changes. 
