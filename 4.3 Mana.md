...
## 4.3.3.1 cMana update procedure

### 4.3.3.1.1 Pledging of the cMana relative to a transaction with timestamp smaller than the last reference time 

The update must be done in two steps, always in the order specified below:

* _a_) **Base cMana update**. Just before updating the base cMana vector we must stored it, since the last base cMana state (that we call $\text{Old\_Base\_cMana}$) and the time relative to the last update are used later for the cMana calculations. This temporary vector can be deleted right after the cMana vector is updated.
...
* _b_) **Pledging and revoking cMana**. Suppose that the last cMana vector is relative to time $t$ and the transaction being added has a timestamp $t-\delta$. 
    
    If the base cMana balance of node $i$ (before we added this transaction) was $\text{Old\_Base\_cMana}(\text{Node}_i)$ and the new base cMana balance (after the addition of this transaction) is $\text{Base\_cMana}(\text{Node}_i)$, then we update the cMana vector adding to all nodes' entry the term:
...
### 4.3.3.1.2 Pledging of the cMana relative to a transaction with timestamp larger than the last reference time

The update must be done in three steps, always in the order specified below:
...
* _b_) **Updating the cMana with respect to time**. Suppose that the outdated cMana $\text{Old\_cMana}$ is relative to time $t-s$ and we want to update it to time $t$. In this case, we update all cMana entries as follows:
...
* _c_) **Pledging and revoking cMana**. Suppose that the last cMana vector is relative to time $t$ and the transaction being added has a timestamp $t-\delta$. 
...
The update must be done as specified below:

*  **Updating the cMana with respect to time**. Suppose that the last cMana vector $\text{Old\_cMana}$ is relative to time $t-s$ and we want to update it to time $t$. In this case, we update all cMana entries as follows:
