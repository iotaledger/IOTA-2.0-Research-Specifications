...
The module presented in this specification, decentralized random number generator (dRNG), allows for the decentralized generation of randomness used in the IOTA 2.0 network. The dRNG protocol is divided into three phases. In the first phase, a committee of high mana nodes is selected. In the second phase, the selected committee members create a distributd public key (DPK) which is used in the third phase where the committee publish random numbrs in the Tangle. In the IOTA 2.0 protocol, the decentralized random numbers are used in the FPC. Possible, future use in other modules like autopeering, rate control or smart contracts are discussed.
...
a) Nodes declare their readinnes to be a committee member with a special ''application'' transaction
b) Selection of $n$ top mana holders 


**2.  DKG PHASE:**

Distributed key generation phase. takes place on the Tangle layer
...
*The committee's size from top mana holders equals $m$; the size of the community committee equals $m_2$.*



3. Committee seats:   $n, n_2$

*The number of identities (seats) in the committee. $n,n_2$ are not equal to $m,m_2$ because certain nodes receive more than one seat, i.e.: node publishes more than one beacon message.*



4. Signature threshold parameter: $f \in [0,1]$ (percentage), $t \in \mathbb{N}$ (number of beacon messages)

*In order to find a random number, nodes require $t$ or more beacon (secret revealing) messages $\left (t =\lfloor nf \rfloor +1 \right )$.*
...
7. Time window of the distributed key generation phase:  $\Delta_C$ 

*The nodes selected for the committee produce distributed private key (DPK) within $\Delta_C$ units of time*



8. Time delay bound on the dRNG messages: $D_{RN}$

*The network does not accept dRNG message with the timestamp off by more than $D_{RN}$ from node's local time (level 3 bad).*
...
7. Time interval for DKG phase $\Delta_C =$ $[min]$
...
MgY29uZmxpY3Rz4oCmIiwic3RhcnQiOjExNTM2LCJlbmQiOjEx
NzAzfSwidkwzakFvR2F1V3RBMDNPNiI6eyJ0ZXh0IjoiU3VjaC
B0cmFuc2FjdGlvbiB3b3VsZCBiZSB1c2VkIHRvIGV4dHJhY3Qg
Km9ubHkqIGEgbWFuYSB2YWx1ZSBvZiBhbiBpc3N1aW5nIG5vZO
KApiIsInN0YXJ0IjoxMDg3NSwiZW5kIjoxMTA1NH0sIjNNZ09O
RjBEcW5lRWdUbmsiOnsidGV4dCI6ImRlY2xhcmVkIG1hbmEgIC
AgICAgICAgICAgICAgICAgIHwiLCJzdGFydCI6MjE1MDUsImVu
ZCI6MjE1MDV9LCIwRG16aGJkbnVybEF1T2NLIjp7InRleHQiOi
JXZSBjYW4gYmUgc3VyZSB0aGF0IGFmdGVyIHRoZSB0aW1lICR0
XzAgLSBcXERlbHRhX0MkIG5vIGFwcGxpY2F0aW9uIG1lc3NhZ2
Ugd2lsbOKApiIsInN0YXJ0IjoxMDQ4NywiZW5kIjoxMDU5M30s
...
JzdGFydCI6MjE1MDUsImVuZCI6MjE1MDV9fSwiY29tbWVudHMi
...
RvcnkiOls3NTY0MDA2NjIsNDE4NDA3OTA3LC0yNDgxMTk0NDUs
LTEwNzAwMzE1MDEsLTY1OTUwMTYxN119
-->
