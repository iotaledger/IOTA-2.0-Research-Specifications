...
The module presented in this specification, decentralized random number generator (dRNG), allows for the decentralized generation of randomness used in the IOTA 2.0 network. The dRNG protocol is divided into three phasewo parts. In the first phase, a committee of high mana nodes is selected. In the second phase, the selected committee members create a distributd public key (DPK) which is used in the third phaseart, we provide a committee selection method. The committee is selected from the high mana nodes. We do not assume perfect agreement on the mana values  (different nodes can have slightly different opinions on the mana values of other nodes). In the second part, the committee members use a verifiable secret sharing and the threshold signature scheme to produce a random number. An example of such protocol is *drand* where the committee publish random numbrsmembers publish *beacon messages* which contain part of the randomness. Those messages are published in the Tangle. In the IOTA 2.0 protocol, thenetwork, decentralized random numbers are used in the FPC. Possible, future use in other modules like autopeering, rate control or smart contract, rate control, and certain autopeering protocols (are discussed.-row). 
...
a) Nodes declare their readinnes to be a committee membermana with a special ''application'' transaction
...
Distributed key generation phase. takes plac 
Key signing phase on the Ttangle layer
...
*The number of identities (seats) in thein the committee from top mana holders equals $n$; the number of identities in the pre-selected committee. equals $n,n_2$ are.* $n$ is not equal to $m,m_2$ because certain nodes receive more than one seatidentity, i.e.: node publishes more than one beacon message.*  



4. SignatureDrand threshold parameter: $f \in [0,1]$ (percentage), $t \in \mathbb{N}$ (number of beacon messagnodes)

*In order to findproduce a random number, nodes require $t$ or more beacon (more or exactly $t$ secret revealing (beacon) messages $\left ($t =\lfloor nf \rfloor +1 \right )$).*
...
76. Time window of the distributed key generation phaseinterval of committee ''application'' submission:  $\Delta_A$ 
...
*The nodes selected forto the committee should produce distributed private key (DPK)key within $\Delta_C$. units of time*




8. Time delay bound onf the dRNG messages: $D_{RN}$

*The network does not accept the dRNG messages with the timestamp off by more than $D_{RN}$ from node's local time (level 3 bad).*
...
MgY29uZmxpY3Rz4oCmIiwic3RhcnQiOjEyODIwLCJlbmQiOjEy
OTg3fSwidkwzakFvR2F1V3RBMDNPNiI6eyJ0ZXh0IjoiU3VjaC
B0cmFuc2FjdGlvbiB3b3VsZCBiZSB1c2VkIHRvIGV4dHJhY3Qg
Km9ubHkqIGEgbWFuYSB2YWx1ZSBvZiBhbiBpc3N1aW5nIG5vZO
KApiIsInN0YXJ0IjoxMjE1OSwiZW5kIjoxMjMzOH0sIjNNZ09O
RjBEcW5lRWdUbmsiOnsidGV4dCI6ImRlY2xhcmVkIG1hbmEgIC
AgICAgICAgICAgICAgICAgIHwiLCJzdGFydCI6MjIzOTksImVu
ZCI6MjIzOTl9LCIwRG16aGJkbnVybEF1T2NLIjp7InRleHQiOi
JXZSBjYW4gYmUgc3VyZSB0aGF0IGFmdGVyIHRoZSB0aW1lICR0
XzAgLSBcXERlbHRhX0MkIG5vIGFwcGxpY2F0aW9uIG1lc3NhZ2
Ugd2lsbOKApiIsInN0YXJ0IjoxMTc3MSwiZW5kIjoxMTg3N30s
...
JzdGFydCI6MjIzOTksImVuZCI6MjIzOTl9fSwiY29tbWVudHMi
...
RvcnkiOlsxMzU2NjQzNjgxLDc1NjQwMDY2Miw0MTg0MDc5MDcs
LTI0ODExOTQ0NSwtMTA3MDAzMTUwMSwtNjU5NTAxNjE3XX0=
-->