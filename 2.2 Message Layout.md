# 2.2 Message Layout
This section describes the *Message Layout*, its syntactical validation and additional metadata that needs to be stored by a node for a message.
...
## Metadata
Next to a message itself, a node needs to store additional data that describes its local perception of a message and is not part of the Tangle.
