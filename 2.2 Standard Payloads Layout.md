...
| -----| ------ | ----------- |
|`MAX_PAYLOAD_SIZE` | The maximum allowed payload size in kb.| 32 or 64 |
...
The `Deal messages` exchanged to produce a public/private collective key during the DKG phase. The `Deal messages` are issued by the nodes that qualified for the dRNG committee participation (see [dRNG](./6.6%20dRNG.md) (distributed Random Number Generator)).
