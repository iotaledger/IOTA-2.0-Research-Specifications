...
- `MAX_PAYLOAD_SIZE` The maximum allowed payload size that can be defined by the user. (To be decided between 32kb and 64kb ???)
...
The salt declaration payload is used by node to declare their initial salt. In a salt declaration message, the declaring node includes
...
<table>
    ...
    <tr>
        <td>NodeID</td>
        <td>uint32</td>
        <td>The declaring node ID (which can be different from the node ID issuing the message).</td>
    </tr>
    <tr>
        <td>Salt</td>
        <td></td>
        <td>The initial salt.</td>
    </tr>
            <tr>
    ...
    <tr>
        <td>Timestamp</td>
        <td>time</td>
        <td>The timestamp (which should be close to the timestamp of its containing message).</td>
    </tr>
    <tr>
        <td>Signature</td>
        <td></td>
