...
Payloads may contain arbitrary data up to `MAX_PAYLOAD_SIZE` and that allows to build additional protocols on top of the base protocol in the same way as TCP/IP allows to define additional protocols on top of its generic data segment.
...
Each payload shall be described by the `uint32` **payload type** field. To separate user-defined payloads from essential core payloads and allow future extension of the protocol, the first (last ???) four places (types 0-255) are reserved for core payload definitions, and all user defined payloads that does not restrict this rule shall be discarded.

Additionally all payloads shall start with following fields with a respect to presented order:
...
- `MAX_PAYLOAD_SIZE` The maximum allowed payload size that may be defined by the user. (To be decided between 32kb and 64kb ???)
...
All core payloads along with its typesare listed in the following table
...
<table>
    ...
    <tr>
        ...
        <td>uint32</td>
        <td>
        Set to <strong>???</strong> to denote a <i>FPC statement Payload</i>.
        </td>
    </tr>
    <tr>
        <td>Version</td>
        <td>uint8</td>
        <td>Version of the FPC statement payload.</td>
    </tr>
    <tr>
...
<table>
    ...
    <tr>
        <td>Payload Type</td>
        <td>uint8</td>
        <td>Set to <strong>???</strong> to denote a <i>Collective Beacon</i> payload or to <strong>???</strong> for <i>Beacon</i> payload.</td>
    </tr>
    <tr>
        <td>Version</td>
        <td>uint8</td>
...
A message used by node to declare its willingness to participate in Committee Selection.
...
<table>
    ...
    <tr>
        ...
        <tr>
            <td>Payload Type</td>
            <td>uint8</td>
            <td>Set to <strong>???</strong> to denote a <i>Application Message</i> payload.</td>
        </tr>
        <tr>
            <td>Version</td>
            <td>uint8</td>
...
The `Deal messages` exchanged to produce a public/private collective key during the DKG phase.
...
<table>
    ...
    <tr>
        ...
        <tr>
            <td>Payload Type</td>
            <td>uint8</td>
            <td>Set to <strong>???</strong> to denote a <i>Deal Message</i> payload.</td>
        </tr>
        <tr>
            <td>Version</td>
            <td>uint8</td>
    ...
    <table>
        ...
        <tr>
            <td>Nonce</td>
            <td>ByteArray</td>
            <td>A nonce used in AES-GCM.</td>
        </tr>
        <tr>
            <td>EncryptedShare</td>
            <td>ByteArray</td>
            <td>A ciphertext of the share.</td>
        </tr>
        <tr>
            <td>Threshold</td>
            <td>uint32</td>
            <td>A threshold of the secret sharing protocol (decided during committee selection).</td>
        </tr>
        <tr>
            <td>Commitments</td>
            <td>ByteArray</td>
            <td>Commitments of the polynomial used to derive the share.</td>
        </tr>
    </table>
    </details>
    </tr>
...
### Salt decalaration payload

The salt declaration payload is used by node to declare their initial salt. In a salt declaration message, the declaring node includes following fields:
...
<table>
    ...
    <tr>
        <td>Payload Type</td>
        <td>uint8</td>
        <td>Set to <strong>???</strong> to denote a <i>Salt declaration</i>.</td>
    </tr>
    <tr>
        <td>Version</td>
        <td>uint8</td>
    ...
    <tr>
        <td>NodeID</td>
        <td>uint32</td>
        <td>The declaring node ID which may be different from the node ID issuing the message.</td>
    </tr>
    <tr>
    ...
    <tr>
        <td>Timestamp</td>
        <td>time</td>
        <td>The timestamp that shall be close to the timestamp of its containing message.</td>
    </tr>
    <tr>
        <td>Signature</td>
        <td></td>
