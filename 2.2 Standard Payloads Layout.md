...
Each payload shall be described by the `uint32` **payload type** field. To separate user-defined payloads from essential core payloads and allow future extension of the protocol, the first (last ???) four places (types 0-255) are reserved for core payload definitions, and all user-defined payloads that do not restrict this rule shall be discarded.
...
The ledger state is changed through transactions payloads or value transfers.
...
<table>
    ...
    <tr>
        <td>Version</td>
        <td>uint8</td>
        <td>Theh version of the FPC statement payload.</td>
    </tr>
    <tr>
