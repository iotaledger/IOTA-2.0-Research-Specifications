...
The module presented in this specification, resynchronization, allows for the node's resynchronization. Desynchronisation might be a result of the failure of the consensus mechanism.  Currently, the consensus mechanism is Fast Probabilistic Consensus (FPC), for more see [FPC-BI: Fast Probabilistic Consensus within Byzantine Infrastructures"](https://arxiv.org/abs/1905.10895) and [specification of FPC modue](https://hackmd.io/s/HkFbpbTrU). Researchers at IF researched FPC thoroughly and deduced that this consensus mechanism is secure even in the Byzantine environment. However, due to its probabilistic nature, it is possible that with certain small (although nonzero) probability, several nodes finalize FPC on the opposite opinion than the rest of the network. For the details of such an event, we recommend reading through the simulations papers:  ["Robustness and efficiency..."](https://arxiv.org/abs/1911.08787) and ["Fast Probabilistic Consensus with Weighted Votes"](https://www.overleaf.com/project/5e3a96c9ebfeb20001821bb5) where this probability and improvements to the protocol are discussed.



The resynchronization role is to provide an automatic response to such an event. Without this, the only alternative would be to manually "kill" the node and set it up again. 



Note that the resynchronization mechanism can, in fact, "overwrite" FPC as it is a new finalization procedure on top of FPC. This opens the protocol for new attack vectors. With this in mind, we designed the resynchronization as a procedure started only when very strong evidence suggests that the node is out of synch. 
...
During the first one, node gathers evidence of potential falling out of synch. If this is the case, the node tries to confirm the possible falling out of synch. This is done by the query of the majority of the network. If the result of the query confirms the findings of the trigger step, then the node resynchronizes. 
...
B0aGUgY3VycmVudOKApiIsInN0YXJ0IjoyNTg0LCJlbmQiOjI4
NjB9LCJYWk9Qa041VWhnbU1wR3V4Ijp7InRleHQiOiJgaW5jb2
1fdHhfcmVhbGl0eWAgVHJhbnNhY3Rpb24uIFRyYW5zYWN0aW9u
IGFzc29jaWF0ZWQgd2l0aCBjdXJyZW50bHkgdXNlZCByZWFs4o
CmIiwic3RhcnQiOjMwODYsImVuZCI6MzE2OX0sInFZQXdtTWUx
ZlNpbmh6ZE0iOnsidGV4dCI6Im1lc3NhZ2UgdHlwZSAgICAgIC
AgICAgICAgICAgICAgIHxcbnwiLCJzdGFydCI6NzQ1NiwiZW5k
Ijo3NDU2fX0sImNvbW1lbnRzIjp7ImFhN0w0dTN3WkswN1d4T0
