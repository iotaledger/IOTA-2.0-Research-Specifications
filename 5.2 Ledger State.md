...
Note that the `branchID` of an aggregated branch is always created from the original referenced `branchID`, rather than any direct referenced aggregated `branchID`s. Thus aggregated branches have no further branches as their children and they remain tips in the branch DAG. 
...
### 5.2.3.4 Approval weight propagation
...
??? What happens to the dis-confirmed branches and their children. What happens to the messages? Approval switch ?
