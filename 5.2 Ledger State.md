...
### 5.2.1 Parallel reality based ledger state 

Coordinate branches with UTXO

The Tangle uses the ledger state to assign branchIDs to the messages -> use Hans' picture as an example

Nodes can only add their approval weight to branches which they consider to be the correct branch, since approval weight propagates monotonically to the indirectly referenced messages. Thus, it would be possible that messages cannot gain approval weight if they attach to the wrong part of the Tangle. To overcome this limitation, we introduce the concept of weak reference, with which the approval weight is only assigned to the referenced message and thus the branch of that message itself, see ???
