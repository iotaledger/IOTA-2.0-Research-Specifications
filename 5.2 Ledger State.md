...
FUNCTION normalizedBranches = NormalizeBranches(initialBranches)
    ...
    WHILE Len(parentsToCheck) != 0
        ...
        # if branch check fails, i.e. a conflict set was seen twice, return a nil list
...
Since messages in [the Tangle ???]() are dependent on the fate of the messages they approve, we *shall* create dependencies between messages and the branches that their parents belong to.
More specifically on solidification the branches of the approved parent messages are collected and the function `NormalizeBranches()` is applied. 

If the returned value is `NULL` the message is rejected since it is not valid. 
Otherwise, the message is added to the returned `branch` container if only one branch is returned. If the number of returned branches is larger than one we apply `GetAggregatedBranchID()` to determine the aggregated branch container to which the message should be added. 

Furthermore, if the message itself is a value message, i.e. a transaction, we check whether the transaction is member of a conflict set, see section [5.2.2.1](5.2.2.1%20Conflict%20sets%20and%20detection%20of%20double%20spends). If this is the case a new branch is created instead and assigned to the transaction. If the other member in the conflict set has not been assigned yet a branch, also that transaction is assigned a new branch and all transactions and messages that have dependencies to this transaction are updated accordingly.
