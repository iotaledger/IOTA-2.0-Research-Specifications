...
Since messages in the Tangle are dependent on the fate of the messages they approve, we *shall* create dependencies between payloads, messages, and branches.  The branch IF of a message or a transaction represents all the conflicts upon which that object depends.  Specifically, we associate a branch to a payload and to a message in the following way.  


- The branch of a non-value payload is always the master branch.
- The branch of a transaction then:
    - If the transaction is a conflict, then it is assigned a new branch whose branchID of the transactionID.
...
This assignments captures the essence of weak and strong parents, see [Tip Selection Specification](./4.3%20Tip%20Selection%20Algorithm.md).  Strong arrows pick up the dependencies of the whole past cone, where as the weak arrows only penetrate to the paylaod of the parent, ignoring the history of the parent.  

After a message is solidified, it and its paylaod are both assigned their branch.  During this check, the message is flagged as invalid if
