...
If there is more than one consumer in the consumer list we *shall* create a conflict set list `conflictSet`, which is identical to the consumer list. The `conflictSet` is uniquely identified by the unique identifier of the output `outputID`. 
...
More specifically a container `branch` *shall* be created for each transaction that double spends an output. Every transaction that spends from a transaction in a given `branch`, i.e. is in the future cone in the UTXO DAG of the double-spending transaction that created `branch`, is also contained in this `branch`.
...
??? Are branches that are confirmed by consensus merged back into Master? Are the conflicting branches now deleted?
