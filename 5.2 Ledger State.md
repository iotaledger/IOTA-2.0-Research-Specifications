...
A message inherits the branches of its parents. Thus, every message indirectly approves and references every branch in its past cone.
By attaching to certain messages the issuing node is effectively also attaching to certain branches. This can be utilized to efficiently make their opinions public about which parts of the Tangle they consider to be valid, see also [**3.2.3.3** Tip selection](). 
Individual nodes can only add their approval weight to branches which they consider to be the branch that should be accepted. 
...
However, if messages attach to the part of the Tangle that would become disqualified for the tip selection, they cannot gain enough approval weight to become confirmed. 
To overcome this limitation, we introduce the concept of weak reference, with which the approval weight is only assigned to the referenced message and thus the branch of that message itself, see also the discussion on weak parents in section [approval switch / strong parent section ???](). -->

### 5.2.3.6 Merging of branches into the master branch

Through the method described in the previous section, messages gain approval weight. Once the approval weight exceeds a certain threshold we consider the message as confirmed, see also section [**3.2.3.4** Finality]().
