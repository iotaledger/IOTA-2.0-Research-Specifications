...
In the simplest case the already existing transaction does not have any future Markers assigned and is not a Marker itself. Thus it becomes a Marker itself, with a new Marker-Sequence associated.
Second, the already booked transaction has assigned a Marker already. Then the existing Marker-Sequences get re-mapped starting from that Marker.
