...
If a transaction arrives that double spends an already booked transaction, a new Marker-Sequence shall be created for the newly arrived message (containing the transaction). 

For the already booked conflicting transaction two scenarios can occur. Note that in both scenarios **no** new Marker is created since the transaction is already booked and since the Marker DAG shall not be changed. 

First, assume the existing booked transaction is a Marker itself. Then the Marker-Sequence in which this Marker is part of is being split. The Marker gets mapped onto the new branch and all referencing Markers get remapped to account for the new branch mapping. 

Second, assume the existing transaction is not a Marker. Then all messages between the transaction and the Future Markers (including the transaction itself) get mapped individually to the new branch mapping. From the Future Markers onwards, the same applies as in the first scenario.
