...
Messages are selected from the strong tips set at random and according to certain rules (see below). If there is no strong tip the selection happens on the weak tips set instead. A selected message is assigned as a Martker, which we call **Marker**. A Marker consists of the following data
...
Each message maintains metadata that provides information about the Markers that are (directly) in the past or future cone of the message, as well as whether the message itself is a Marker and what rank the message has:
...
<table>
    ...
    <tr>
        ...
        <td>A list of PMs from different Clusters in the past cone of the message.</td>
    </tr>
    <tr>
        <td><code>FutureMarkers</code></td>
        <td>map[<code>Cid</code>]<code>Mid</code></td>
        <td>A list of FMs from different Clusters in the future cone of the message.</td>     
    </tr>
</table>
<br>
