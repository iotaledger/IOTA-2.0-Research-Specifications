...
* more than a certain number of messages (`maxMsgPerMarker`) have been received since the last Marker.  If there are several large clusters, this rule must be applied per cluster.
* a certain time window (`maxTimePerMarker`) has passed since the last Marker. If there are several large clusters, this rule must be applied per cluster.
...
Whenever a new Cluster is created, we assign 
- a new `Cid`, created by the rule $new\texttt{Cid}=1+last Cid$. 
- a new $\texttt{CR}=1+max(\text{referenced }\texttt{CR})$. 
To prevent assigning a new `Cid` when combining the same Clusters again, we build parents-child relationships in a map if a new Cluster is created. 
