...
<table>
    ...
    <tr>
        <td><code>SID</code></td>
        <td>uint64</td>
        <td>Unique Marker-Sequence identifier</td>
    </tr>
    <tr>
        <td><code>MR</code></td>
        <td>uint64</td>
...
### 4.7.4.3 Update on double spends

If a transaction arrives that double spends an already booked transaction, a new Marker-Sequence shall be created for the already booked Marker. For the Markers DAG and Message DAG two scenarios can occur. Note that in both scenarios **no** new Marker is created since the messages are already booked and since the Marker DAG shall not be changed. However, the Marker-Sequence DAG and the associated Marker-Sequence ranks can be altered and changed.

First, the existing booked transaction is a Marker itself. The Marker-Sequence in which this Marker is part of is being split, creating a new Marker-Sequence. Then the Marker gets mapped onto the new sequence and all referencing Markers get remapped to account for the new Marker-Sequence. The Sequence Rank can be adjusted simultaneously when updating the referencing Markers.
Second, the existing transaction is not a Marker. Then all messages including and between the transaction and the Future Markers get mapped individually to the new Marker-Sequence. From the Future Markers onwards, the same applies as in the first scenario.
