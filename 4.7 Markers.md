...
* **Future Marker (`FM`):** This field in the message metadata is (potentially) updated when a new Marker is generated in the future cone of the message. It contains the list of Markers for which there is no other Marker between the Marker in question and the message. For more details, see the rules defined in Section "Metadata of messages".
...
Messages are selected from the strong tips set at random and according to certain rules (see below). If there is no strong tip the selection happens on the weak tips set instead. Such a selected message is assigned as a Marker, which we call **Marker**. A Marker consists of the following data
