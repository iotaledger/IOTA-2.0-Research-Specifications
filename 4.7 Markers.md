...
If a transaction arrives that double spends an already booked transaction, a new Marker-Sequence shall be created. For the Markers DAG and Message DAG several scenarios can occur.  

In the simplest case the already existing transaction does not have any future Markers assigned. Thus it becomes a Marker itself. 
Second, the already booked transaction has assigned a Marker already. Then the existing Marker-Sequences gets re-mapped starting from that Marker.
Third, if the double spend is with a transaction that is already booked but is not a marker itself, the Marker tool shall not create a new Marker for that value message. The transaction and all messages in between the transaction and the future Markers shall be mapped individually to the new Marker-Sequence.
...
Through this approach past and future Markers do not cross weak parents.
