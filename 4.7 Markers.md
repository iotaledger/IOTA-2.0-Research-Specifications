...
[CHECK THIS:] The Marker tool achieves this by defining a parallel internal data structure, consisting of additional metadata applied to specific messages in the Tangle, to aid the Node in making determinations about those messages. The additional data defined for Marker messages are not themselves posted to the Tangle but make up a parallel data structure maintained within the Marker tool. 
...
* **Rank:** For a given DAG the rank is defined as an increasing index for the vertices of that DAG. Specifically, if a vertex $A$ references $B$ and $C$ then $rank(A)=max(rank(B),rank(C))+1$.
...
Messages are selected (as defined in Section XXX) from the strong tips set at random or according to certain rules (see below). If messages are selected from the tips set the following rule shall be applied by the Marker tool: if there is no strong tip the selection shall be from message in the weak tips set. A selected message is linked to a Marker within the Markers tool, called **Marker**. A Marker consists of the following data:
...
* more than a certain number of messages (`maxMsgPerMarker`) have been received since the last Marker.  If there are more than [some number] Marker-Sequences over a certain size [define 'large'], this rule must be applied per Marker-Sequence.
* a certain time window (`maxTimePerMarker`) has passed since the last Marker. If there are [some number] [define 'large'] Marker-Sequences, this rule must be applied per Marker-Sequence.
