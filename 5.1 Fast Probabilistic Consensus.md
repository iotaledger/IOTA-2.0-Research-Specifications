...
Note that since mana changes over time, FPC may use different mana for different `objectID`s that are voted on at the same time. For an `objectID`, FPC must use the mana list `consensusManaList` obtained by `GetActiveConsensusMana(timeObjectID)`, where  `timeObjectID` is the timestamp `ObjectID`.
