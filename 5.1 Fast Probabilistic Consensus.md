...
| `DRNG_WAITING_TIME` | double |  Maximal waiting time (in seconds) to receive dRNG number|
...
| Name | Type |Description |
| -----| ------ | ----------- |
| `opinion` | nulled boolean | Opinion of the validity the `objectID`; `TRUE` corresponds to `LIKE` and `FALSE` to `DISLIKE`|
...
6. A node must continue to reply to queries as long as `AnswerStatus(type, objectID)=TRUE`, where `type` is the type of the `objectID`. If `AnswerStatus(type, objectID)=FALSE` the variable `answerStatus` must be updated accordingly.
...
Note that since mana changes over time, FPC may use different mana for different `objectID`s that are voted on at the same time. For an `objectID`, FPC must use the active consensus mana of the epoch that contains the timestamp of the `objectID`. We refer to [Section 4.3 Mana](#) for the specification of active consensus mana. We use a generic function `GetActiveConsensusMana(time)` that retrieves the active consensus mana with respect to a given time `time`.
...
>[Seb] to be discussed/related with DRNG,  
...
The queries must follow the following layout. 
...
In the case that a certain `objectID` is unknwon to the node, it must reply with `NULL`. If the `objectID` is currently treated by FPC a node must reply  with the current opinion, otherwise, retrieve the `opinion` from the `opinionField` of the `objectID`. If the `opinionField` is `NULL` the node must reply with `NULL`.  
...
A node shall only reply to queries if all included `objectsID` have status `answerQuery=TRUE`.
