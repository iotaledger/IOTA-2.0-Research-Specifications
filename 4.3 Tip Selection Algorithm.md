...
- **Tip**: A message is considered a tip (by a node) if it is selectable by the Tip Selection Algorithm, i.e. it is an element of any _Tip Pool_ (of that node).
...
Observe that since validity depends only on objective filters, its classification is shared among all the nodes, on the other side eligibility could have discrepancies as it depends on the local view (clock) of a node. 
...
Differently from the legacy implementation, we will not have a single pool, but instead two, divided according to the new concept of the _Approval Switch_. 
...
- **Branch:** A non-conflicting past cone of some output in the UTXO DAG. 
