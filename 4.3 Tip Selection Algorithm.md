...
Although we will not be using here, the new message layout (more information in Section 2.2 - Message Layout) has the capabilities for a message to have a non-fixed number of messages that it approves (parents), which range from two to eight (see [Section 4.8 - Message Creation](https://github.com/iotaledger/Coordicide-Specifications/blob/main/4.8%20Message%20Creation.md)). This can be used to develop tip spam protection mechanisms among other purposes. We will denote the number of parents being used in the algorithm by _Parental Number_. 
...
In order to properly define and explain the approval switch mechanism, we will need some extra defintions. We also refer to the basic concept of branches ([Section 5.2 - Ledger State]()) and validity ([Section 4.1 - The Tangle]()), as they are required to the understanding of some concepts below.  
...
- **Strong approval**: A binary field in the message associated with each of its parents, filled by its issuer node. In general it represents that the issuer node declares that the message and its entire past cone are liked. 
- **Weak approval**: We say message $x$ weakly approves message $y$ if the issuer node from $x$ asserts that $y$ contains a liked payload, but its past cone is not completely liked. 
...
3. **Liked Payload Pool:** This pool consists of all eligible messages that contain a payload tagged as "liked". 
...
1. **Removal**: Tips are removed when they are approved by other messages. This can happen in two ways:
     - When the node issues a message, the selected tips will be removed from the respective tip pools after the Tip Selection Algorithm is performed (we briefly explain the procedure in the [R-URTS](#R-URTS) subsection, further information may be found in the [Section 4.8 - Message Creation]()).
