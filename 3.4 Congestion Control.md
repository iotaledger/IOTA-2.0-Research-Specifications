...
<p>We use a FIFO policy to drop messages in a fixed-size outbox buffer. More efficient policies are currently under research.</p>
<pre><code>    msgDrop = bufferQueue.head()
    
    if msgDrop.nodePtr == NULL:
        activeNode[msgDrop.nodeId].remove()
    else:
