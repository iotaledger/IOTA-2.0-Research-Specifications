...
* _Message weight_. Weight of a message is used to priority messages over the others and it is calculated depending on the type of message and of the message length, as described in Section 2.4.
...
*   `int SCHEDULING_INTERVAL`: clock time interval between subsequent executions of the function `Schedule()`
...
*   `int W`: time window (see Section 4.2 - Timestamps)

### 3.4.3.2 Local variables

*   `float ownRate`: issuance rate of `ownID` according to the rate setter
*   `list activeNode`: updated list of nodes having at least one message in the outbox queue (more details in Section 3.4.3.5)
*   `queue bufferQueue`: actual outbox queue where messages are ready to be scheduled (more details in Section 3.4.3.5)
...
*   `stopUpdates`: time interval during which rate setter is not updated
...
* `WorkCalculator(x)`: provides the weigth of message `x` (see Section 2.4)
...
```
>>>>>>> 0686f59bb05136a3834dc271a120c28a843aab90
...
At regular intervals, i.e., every `SCHEDULING_INTERVAL` time units, the function `Schedule()` selects the next message to gossip and process, if at least one message exists in `bufferQueue`. Otherwise, it returns `NULL` and the scheduling slot is missed. The local variable `pauseUpdates` is initialized to _0_.
```vbnet
### periodically every SCHEDULING_INTERVAL time units ###
...
FUNCTION msg = Schedule()
    IF Len(bufferQueue) > 0
        ...
        WHILE activeNode[nextID].deficit < Weight(msg) OR msg.timestamp > CurrentTime() + W OR Parents(msg) have not been scheduled
...
## 3.4.4 Under research (not so relevant, we could remove...)

*   _Minimum mana requirement._ Nodes must hold a sufficient amount of mana (larger than `MIN_MANA`) to be able to successfully issue new messages. We are currently investigating a way to adapt this threshold over time, depending on the current traffic congestion of the network.
...
* _Max buffer capacity._ We are investigating how to properly design buffer capacity `MAX_BUFFER` to achieve an optimal tradeoff between efficiency (low delay) and consistency (few message drops).
