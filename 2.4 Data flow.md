# 2.3 Data flow
This section provides a high-level description of the interaction between components of the IOTA protocol. The protocol can be divided into three main elements: a P2P overlay network, an immutable data structure, and a consensus mechanism, which are wired together via game-theoretical incentives. In the IOTA protocol, these three elements are abstracted into layers, where—similarly to other architectures—upper layers build on the functionality provided by the layers below them. The definition of the different layers is merely about the convenience of creating a clear separation of concerns.

![](https://i.imgur.com/IcKNbez.png)
## 2.3.1 Network Layer
The network will be maintained via the Network layer, which can be characterized as a pure P2P overlay network, meaning that it is a system that runs on top of another network (e.g., the Internet), and where all nodes have the same roles and perform the same actions (in contrast to client-server systems). IOTA's Network Layer consists of two basic modules: the [peer discovery module](./3.3%20Peer%20Discovery.md) (which provides a list of nodes actively using the network), and the [neighbor selection](./3.4%20Neighbor%20Selection.md) (aka autopeering), which actually picks peers. 
## 2.3.2 Communication Layer
The communication layer concerns the information propagated through the network layer, which is contained in objects called messages. This layer forms a DAG made up from messages called [the Tangle](./4.1%20The%20Tangle.md): a replicated, shared and distributed data structure that emerges through a combination of deterministic rules, cooperation, and (either direct or virtual) voting—as FPC and approval weight based finality. To prevent the network from being overloaded, the [rate control](./4.5%20Rate%20Control.md) and [congestion control](./4.6%20Congestion%20Control.md) modules control how many messages can be created and which and when messages are gossiped. Finally, the message gossip itself is managed by the Communication layer as well.
