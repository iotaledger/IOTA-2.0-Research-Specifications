...
ADD AN EXAMPLE OF HOW TO READ THIS AS LUIGI SUGGESTED

CHECK THE ARROWS IN THE CODE 

![](https://i.imgur.com/tmz4jSM.png)
...
2. Parsing and Syntactical Validation: it checks if the message and the payload (when present) are syntactically valid, as defined in [Section 2.2 - Message Layout](./2.2%20Message%20Layout.md) and [Section 2.3 - Payloads Layout](./2.2%20Standard%20Payloads%20Layout.md). If the message does not pass this check, it will trigger either an `MessageRejected` or as a `ParsingFailed` event. If it passes the check, it goes to the next step. 
3. Timestamp Difference Check: it checks if the timestamps of the payload and the message are consistent with each other, i.e., if ```transaction.timestamp+TW >= message.timestamp >= transaction.timestamp```, as defined in [Section 4.2 - Timestamps](./4.2%20Timestamps.md). If the message does not pass this check, a `MessageRejected` event is triggered. If it passes the check, it goes to the next step. 
4. Signature check: it checks if the message signature is valid. If the message does not pass this check, a `MessageRejected` event is triggered. If it passes the check, it goes to the next step. 
...
4. Inputs' branches validity check: it checks if all the consumed inputs belong to a valid branch. This is an objective check; it does not depend on the node's perception of the ledger. If the message does not pass this check, the message is marked as `invalid` and not booked. If it passes the check, it goes to the next step. 

After the objective checks, the following subjective checks are done:

5. Inputs' branches rejection check: it checks if all the consumed inputs belong to a non-rejected branch. Notice that this is not an objective check, so the node is susceptible (even if with a small probability) to have this opinion changed over time. For that reason, if the message does not pass this check, the message is marked as `invalid` but is booked into the Tangle and ledger state (even though the balances are not altered by this message). This is what we call "lazy booking", which is done to avoid huge re-calculations in case of a reorganization of the ledger. If it passes the check, it goes to the next step. 
...
CHECK IF WHILE LAZY BOOKING WE ACTUALLY MARK THE MESSAGE AS INVALID
