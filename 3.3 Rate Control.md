...
The mechanisms described act as an emergency break during spam attacks, by slowing down the rate of messages a node can generate.  For honest nodes, the proof of work should be small enough not to hamper performance.  Finer controls on the access are developed in Section 3.4 - Congestion Control, which regulate network traffic during normal periods of congestion.
