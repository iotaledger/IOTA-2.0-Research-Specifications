...
The mechanisms described act as an emergency break during spam attacks, by slowing down the rate of valid messages a node can send.  For honest nodes, the proof of work should be small enough not to hamper performance.  Finer controls on the access are developed in Section 3.4 - Congestion Control, which regulate network traffic during normal periods of congestion.
...
At time $t =$ `CurrentTime()`, if node `m` wants to issue a new message, it *shall* perform a PoW with difficulty $d_m(t)$ such that
...
When a node `n` receives a message from a neighbor, it shall check that PoW with an appropriate difficulty was performed. Let us assume that node `n` receives a message with difficulty $d_m$ issued by node `m`. To decide whether this message should be discarded or not, node `n` counts how many messages $r_m(t)$ issued by `m` it has received in the last $w$ time units. In accordance with the formula above, the node validates the PoW only if the following condition is satisfied:
...
* `int POW_BASE`: the base difficulty $d_0$;
* `float APOW_RATE`: the adaptation rate $\gamma$ (proposed values [0.1 - 1]);
* `int APOW_WINDOW`: the APoW time window $w$ (proposed values [10 - 60s]).
...
* `long timestamp`: a value declared by the node representing time at which the message has been issued;
* `nodeId nodeID`: identity of the node issuing the message defined as the `blake2b` hash of its public key.;
* `int targetDifficulty`: minimum difficulty needed to pass the APoW verification;
* `bool powCheck`: boolean value which indicates whether the APoW verification is successful or not;
* `nodeID ownId`: identity of the node running the algorithm;
...
This procedure returns the difficulty at which a node should compute the PoW when generating a new message.
