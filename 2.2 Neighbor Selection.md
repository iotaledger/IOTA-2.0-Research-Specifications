 ...
 * `packetExpirationTime`: The time used for the request timestamp validation, if the timestamp is older than this threshold the request is dropped
 * `maxPeeringAttempts`: The maximum number of peering requests retries sent to the selected node before the next salt update.

## 2.2.3.2 Node identities
...
* wait to get a [Peering Response](#Peering_Response) that could be positive or negative. 
    ...
    * If after `responseTimeout` no response is received, try again for a fixed `maxPeeringAttempts`. If not succeed then node filter out the peer from future request until the next salt update or the end of the list of potential neighbors is reached.
...
* verifying that the signature of the [Peering Request](#Peering_Request) packet is valid and discard the request otherwise;
...
## 2.2.3.7 Requests and responses
