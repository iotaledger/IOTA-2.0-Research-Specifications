...
1. Future salts must be unguessable: Otherwise, can mine node ids which reduce the request score. This offers protection for the requesting nodes.
...
In order to connect to new neighbors, each node with ID ownId and public salt ζ keeps a list of potential peers derived via [Mana rank](#Mana_rank) that is sorted by their score d(ownId, ·, ζ). Then, the node sends them peering requests in *ascending order*, containing its own current public salt and a timestamp (its nodeID is already embedded in the [packet](#Packet) containing the peering request). 
...
* accept the peering request by sending bakc a *positive* [Peering Response](#Peering_Response) if either one of the following conditions is satisfied, and send back a *negative* [Peering Response](#Peering_Response) otherwise:
    ...
    * the score defined as *s(ownID, remoteID, ζ_private)* is smaller than the current furtherest accepeted neighbor. In this case, send a [Peering Drop](#Peering_Drop) packet to drop the furtherest accepeted neighbor replaced by the requester peer. 
