...
* checking that the `timestamp` field is fresh (i.e., not older than threshold specified by the node) and discard the packet otherwise;
...
* accept the peering request by sending back a *positive* [Peering Response](#Peering_Response) if either one of the following conditions is satisfied, and send back a *negative* [Peering Response](#Peering_Response) otherwise:
    ...
    * the score defined as *s(ownID, remoteID, priv_salt)* is smaller than the current furtherest accepeted neighbor  (i.e., the neighbor with highest score). In this case, send a [Peering Drop](#Peering_Drop) packet to drop the furtherest accepeted neighbor replaced by the requester peer. 
