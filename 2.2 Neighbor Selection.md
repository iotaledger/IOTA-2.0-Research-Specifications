...
In order to connect to new neighbors, each node with ID ownId and public salt ζ $\zeta$ keeps a list of potential peers derived via [Mana rank](#Mana_rank) that is sorted by their distance d(ownId, ·, ζ). Then, the node sends them peering requests in *ascending order*, containing its own current public salt and a timestamp (its nodeID is already embedded in the [packet](#Packet) containing the peering request). 
...
N0YXJ0Ijo2OTE1LCJlbmQiOjY5MTYsInRleHQiOiLOtiJ9fSwi
...
AyNjY0fX0sImhpc3RvcnkiOlstMzM0OTIzMDgzLC0xMzA1NTcz
NjQxLDExNDY4NTU0MjNdfQ==
-->