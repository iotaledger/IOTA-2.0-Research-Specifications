...
* checking that the `timestamp` field is valid (i.e., not older than a given threshold `packetExpirationTime` specified by the node) and discard the request otherwise;
...
<table>
     ...
     <tr>
         <td>data</td>
         <td>ByteArray</td>
         <td>contains the payload of the request or response (e.g., a PeeringRequest packet).</td>
     </tr>
     <tr>
         <td>public_key</td>
         <td>ByteArray[32]</td>
