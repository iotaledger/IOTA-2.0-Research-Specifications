...
* accept the peering request by sending bakc a *positive* [Peering Response](#Peering_Response) if either one of the following conditions is satisfied, and send back a *negative* [Peering Response](#Peering_Response) otherwise:
    ...
    * the distance defined as *d(ownID, remoteID, ζ_private)* is smaller than the current furtherest accepeted neighbor. In this case, send a [Peering Drop](#Peering_Drop) packet to drop the furtherest accepeted neighbor replaced by the requester peer. 
...
In the following algorithm, we iterate over a data structure called `manaRank` containing the list of the nodes' identity for each mana value. The `targetMana` is the mana value of the node performing the ranking. 

```
Inputs: 
    manaRank: mapping between mana values and the list of nodes' identity with that mana; 
...
Outputs:
    set: the set of nodes' identity to consider for neighbor selection;
