...
## 2.1.4.3 Removal
While verifying a new peer, if no or an invalid *Pong* packet is received after `max_verify_attempts` attempts, that node *shall* be removed from the `known_peer_list`. 
...
Upon successful validation of a received [DiscoveryResponse](#DiscoveryResponse) packet, a node *shall* add the nodes contained in the `peers` field to the top ('head') of the `known_peer_list`.
