...
The amount of mana a node has ultimately decides how many it can issue. However, because of the utilization requirement,   computing the exact aloud rate is a bit complicated.  Indeed, utilization ensures the scheduler is always running and that the network resources are used. For example, suppose nodes A,B,C,D each have 100 mana, and they are the only nodes issuing messages.  Then each node can issue 25% of the messages.  Now suppose, node D drops offline, then because of the utilization property,  nodes A,B,C, can issue 33% of the transactions.  Furthermore, if one of the nodes, say C does not issue its full rate, then 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjMzNDA5NDEzLDkyOTE2MjA3NCwyMDIxMz
YxMjk4LDYzOTc5ODEzNywtMTEwMzQ3MDcwMCwtMjEyNDAxOTg1
LDE0NzgyNjMxODEsLTEzNjEzMzU5MTAsNjE5MjMzNTUxLC0xMT
